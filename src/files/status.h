 ...
 * Every field may be NULL — the core then runs silently and never aborts.
 * ------------------------------------------------------------------ */
...
/* Called once per ROM, after work finishes, with what actually happened.
 * detail is a short human-readable line: the game's title on success, or
 * the reason nothing was downloaded. */
typedef void (*ResultFn)(ScrapeStatus status, const char *rom_name,
...
/* Bundled so adding a callback later does not change every signature. */
typedef struct {
    ProgressFn    on_progress;
    ResultFn      on_result;
    ShouldAbortFn should_abort;
    void         *user; /* passed back to each callback untouched */
} ScrapeCallbacks;
